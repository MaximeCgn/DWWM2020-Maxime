...
class DepartementsManager 
{
	...
	public static function getByIdRegion($idRegion)
	{
		$db=DbConnect::getDb();
		$idRegion=(int) $idRegion;
		$q=$db->query("SELECT * FROM Departements WHERE idRegion =".$idRegion." ORDER BY LibelleDepartement");
		while ($donnees = $q->fetch(PDO::FETCH_ASSOC))
		{
			$json[]=$donnees;
		}
	...
	static public function getListAPI()
	{
		...
		$q = $db->query('SELECT * FROM Departements ORDER BY LibelleDepartement');
